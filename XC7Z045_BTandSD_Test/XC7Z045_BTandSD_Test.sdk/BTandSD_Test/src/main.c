...
static u32 data_cnt = 0;			//数据计数器
int txt_cnt = 0;					//文本文件计数器
int pic_cnt = 0;					//图片计数器
char FILE_NAME[30];
				...
				if(check_sum_flag == 0){
					...
					if(data_length != 0xF8){ //当有效数据长度不是0xF8时,data_cnt置为0,暂时没有考虑数据长度是0xF8倍数的问题
						if(data_cnt > 0 && file_type > 0){
							if(file_type == 1){//文本
								//xil_printf("the text data is:\r\n");
								//for(int i = 0; i < data_cnt; i++){
								//	xil_printf("%c",DataBuffer[i]);
								//}
								//xil_printf("\r\n");
								xil_printf("Receive text complete, the text size is %d\r\n", data_cnt);
								itoa(txt_cnt, FILE_NAME, 10);
								...
								//xil_printf("the picture data is:\r\n");
								//for(int i = 0; i < data_cnt; i++){
								//	xil_printf("%d ",DataBuffer[i]);
								//}
								//xil_printf("\r\n");
								xil_printf("Receive picture complete, the picture size is %d\r\n", data_cnt);
								itoa(pic_cnt, FILE_NAME, 10);
								...
								xil_printf("the user defined data is:\r\n");
								for(int i = 0; i < data_cnt; i++){
									xil_printf("%c",DataBuffer[i]);
								}
								xil_printf("\r\n");
...
int main()
{
	...
	xil_printf("All initial completed!\r\n");
